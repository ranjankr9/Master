...
1. js/storeLocator.js  
	...
	1.3 Search implementation : 
	   A. As per current BAIDU GIO code API , It only provides GIO code translation into co-ordinate/location for specially addresses of locaitons in main land
		  China, It does not provide complete search feature based on City/state/country/postal code. So in order to overcome this serach limitataiton below solutions has been taken into consideration. 
          A.1 : To user BAIDU GIO code API to get the location for search input provided by user if this API provies the locaiton then we show the location on 	the map otherwise got for STEP A.2
          A.2 Use Google GIO code web service API to get the locaiton and populate on the map.
